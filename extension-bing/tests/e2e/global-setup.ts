 ...
 *   `e2e_seed_user.py --scenario bing-extension-pro`(创建 e2e 账号 + maps_extension
 *   Pro 订阅 + 签发与插件 exchange 同构的 token 对并注册 Redis 白名单),
 *   结果写入 process.env.E2E_BING_AUTH 供登录态 spec 注入 chrome.storage;
...
/** 后端项目根目录:extension-bing/ 与 backend/ 同级父目录下。 */
...
const SEED_SCENARIO = 'bing-extension-pro'
...
export default async function globalSetup(): Promise<void> {
  ...
  try {
    const seed = runSeed()
    process.env.E2E_BING_AUTH = JSON.stringify({
