...
async function doLoad(): Promise<void> {
  ...
  if (cached && Date.now() - cached.fetchedAt < CACHE_TTL_MS) {
    ...
    logger.info('[MapsRemoteConfig] 命中 1 小时内缓存，跳过远程拉取')
    return
  }
  ...
  try {
    ...
    logger.info('[MapsRemoteConfig] 远程配置拉取并覆盖成功')
  } catch (error) {
...
async function writeCache(cache: MapsRemoteConfigCache): Promise<void> {
  try {
    await storageManager.set(STORAGE_KEYS.MAPS_REMOTE_CONFIG, cache)
  } catch (error) {
    logger.error('[MapsRemoteConfig] 写入远程配置缓存失败:', error)
  }
}
