...
const identity = {
  getAuthToken: vi.fn(
    (_details: chrome.identity.TokenDetails, callback?: (result: chrome.identity.GetAuthTokenResult) => void) => {
    const result = { token: 'mock-drive-access-token' }
    if (callback) {
      callback(result)
      return
    }
  ...
  launchWebAuthFlow: vi.fn(
    (_options: chrome.identity.WebAuthFlowDetails, callback?: (responseUrl?: string) => void) => {
