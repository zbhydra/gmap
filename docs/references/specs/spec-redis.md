...
- **一律走 `build_redis_key()`**（`app/utils/redis_key.py`），自动添加 `settings.redis.key_prefix`；该配置在 schema 与模板中固定为 `gmapsexporter`，所有节点使用同一值，不按 `APP_NAME` 改名，环境隔离使用不同 Redis 实例。禁止裸 key、禁止手拼前缀。
