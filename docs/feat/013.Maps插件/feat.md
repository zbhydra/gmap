...
标题栏(名称 + 语言切换)、账号区与用量条(下方表)、支持入口(联系邮箱复制、反馈渠道)、打点:打开即上报 popup_open。

账号区(登录 / 登出;登录走 007 域 v3 统一流程,合同见 `@../007.用户系统/tech-第三方登录.md` §9):

| 元素 | 形式 | 可点击 | 行为 |
| --- | --- | --- | --- |
| 账号名 | 文本(账号区首行) | 否 | 未登录显示 Guest;已登录显示账号 displayName |
| Sign in 按钮 | 按钮(未登录时出现在账号名右侧) | 是 | 经 background 发起浏览器身份登录(官网确认页 → 一次性 code 回跳);发起后进入 loading,失败静默复位;登录成功无需重开 popup,界面即时变为已登录 |
| Sign out 按钮 | 按钮(已登录时同位置) | 是 | 清插件登录态并撤销服务端当前会话,popup 即时回到 Guest 态 |

登录态变化经插件存储监听即时反映(popup 保持打开也刷新);登录后配额自动切账号归属,用量条数据源与展示形态不变。
