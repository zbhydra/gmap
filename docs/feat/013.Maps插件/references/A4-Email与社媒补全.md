...
- 计量口径(2026-08-30 U7 裁决):记录数配额在采集完成边沿按会话计量(U7 usage 通道),enrich 端点**不重复扣减**;竞品「免费也有计量」的策略由 U7 的免费月度额度等价覆盖。
- **U8 已实现(2026-08-30)**:backend `POST /api/client/maps/enrich`(官网抓取 + Email 正则 + 社媒外链,SSRF 防护/缓存/预算),插件 complete 边沿补全写回;仅抓官网首页,搜索兜底与 Plus Code 补全后置(代码已标注)。
