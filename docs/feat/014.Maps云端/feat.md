...
- R2 / AliOSS 多套配置与启用切换纳入 Online 实施计划，配置合同见 `@../008.管理后台/tech-系统设置.md` 的“对象存储配置”。
...
- 整个任务的采集期限为提交后 60 分钟；到期停止未完成关键词的采集，按 0 条收口，保留已经完成的文件和记录数。业务进程重启不延长期限。
- 单个关键词最终取数失败或对象存储失败时，该关键词以 0 条收口，任务继续处理其他关键词。
- 内部保存失败原因与错误 item 数，不向用户增加 Failed 或 Partial 状态。
- 关键词结果报告写入数据库失败时有限重试；重试耗尽后发飞书告警，保留已有任务状态，由业务进程下次启动恢复。
...
- business 启动后恢复当前 `APP_NAME` 的未完成任务；重复执行不重复增加进度或用量。
- 每个任务绑定创建时启用的一套 R2 或 AliOSS 配置，后续执行和下载使用同一存储；切换启用项只影响新任务，MySQL 不保存商家明细。
- 设计容量为每月约 1,200 万关键词 item、6 亿结果记录；item 固定拆为 20 张同构表。
- 对象按功能与业务日期组织，保留期通过存储侧生命周期规则管理；切换后保留旧存储，待旧对象过期再删除旧配置。

## 验收标准

1. Online 用户可以提交关键词任务，刷新或业务进程重启后任务继续收口。
2. 提交后立即异步执行；业务进程重启后恢复未完成 item，重复执行不重复增加进度或记录数。
