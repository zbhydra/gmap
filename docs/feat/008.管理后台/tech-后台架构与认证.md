...
> 关联:`@feat.md` `@tech-管理模块接口.md` `@tech-节点本地与中心入口.md` `@tech-外部API.md`。节点本地管理架构与节点直连鉴权流见 `@../001.节点系统/tech-节点Admin与本地管理.md`。
...
路由守卫:未持有令牌跳 `/login`;持有令牌访问 `/login` 跳首页。`@../001.节点系统` 的节点管理页挂在 `/service-nodes`,其节点 CRUD 接口规格属节点域。
...
节点本地管理架构、节点直连鉴权流(目标节点 401 时回业务服务器续签并重试)、节点本地接口清单属节点系统域,见 `@../001.节点系统/tech-节点Admin与本地管理.md`。本域只声明"业务服务器 vs 节点本地"使用不同依赖这一边界。
...
错误响应在需要时携带 `data`(如限流等待秒数 `wait_seconds`、活跃请求数 `active_request_count`)。具体错误码名称与触发条件见对应菜单技术文档,例如 `@tech-TG客户端.md`。
