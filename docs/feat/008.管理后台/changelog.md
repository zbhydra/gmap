...
## 2026-09-06 订单 / 用户筛选区压缩为一行工具条

**为什么**:筛选区原为拉伸网格,宽屏下输入框 700px+、时间选择器横贯整卡;窄内容区下占据 4-5 行,数据表被挤出首屏(hydra 反馈「我要看的是内容,不是搜索条件」)。

**变更**:

- `OrdersView` / `UsersView` 筛选区改 flex 单行工具条:字段固定窄宽(文本 280 / ID 250 / 下拉 264 / 时间区间 500,含左置 label,`min(X, 100%)` 保证窄内容区收缩不撑破卡片),操作按钮内联行尾右对齐;删除旧 filter-grid 网格与 960 / 560 断点样式。
- 订单页默认只显示订单号 / 用户 ID / 订单状态 / 创建时间,其余五项收进「更多筛选」折叠(`showMoreFilters`);折叠只隐藏控件,隐藏字段已填值仍参与查询提交。合同记录见 `@tech-订单管理.md` 的「前端联动」。
- i18n 新增 `orders.moreFilters` / `orders.lessFilters`(中英);orders E2E 填写折叠字段前先展开,users E2E 不变。

**验证**:vue-tsc + vite build 通过;Chromium E2E orders + users 共 3 条通过;1920 与 1000 宽度浏览器实测:订单页折叠单行、展开约两行(126px 高,旧版约 208px),用户页单行,均无横向溢出。

## 2026-09-05 用户聚合统一三类 Maps 产品

- 用户 profile、类型、i18n 与弹窗改用 `product_kind`,订阅与月度用量均固定三类 Maps;删除 TG 第四行,后台品牌改为 MapsGrab,订单示例使用现役 Maps 商品。
- 定向用户弹窗 E2E、构建与启动通过;真实后端同账号聚合与页面消费通过。完整跨端证据与限制见 [006 变更记录](../006.订阅系统/changelog.md)。

## 2026-09-05 对象存储多配置落地

- 系统设置支持同类型多套 R2 / AliOSS、新增、凭据轮换、启用切换与删除；稳定配置及发布整理合同见 `@tech-系统设置.md` 的“对象存储配置”。
- Admin real 四配置流程与 Chromium E2E、构建和启动通过；真实云存储因缺凭据跳过。完整验收记录见 `@../014.Maps云端/plans/002.Online任务与结果基建.md`。
- 开发库无旧配置行，未做数据整理；生产配置未操作，发布目标库的旧配置仍需按合同检查并整理。
