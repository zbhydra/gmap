...
> 本文负责网站账户工作区的入口、布局和前端消费边界；业务规则归属用户、订阅与 Maps 云端领域。渠道管理入口的页面归属自 `@../011.Pricing页/tech-pricing与自动续费.md` 迁入本文。

## 用户行为

- 全站导航右侧：匿名时显示登录，登录后显示 Dashboard。通过导航登录成功后进入 Dashboard；取消登录留在原页。
- Dashboard 默认打开 Online Scraper，左侧仅三个菜单：Online Scraper、API 管理、订阅管理。菜单具有独立 URL，支持刷新、直接访问和浏览器前进后退。
...
- 创建表单由 `HistoryView.vue` 承载，位于唯一历史列表上方；创建约束、提交与轮询、首页登录后丢弃预览规则统一见 [首页预览与任务创建](../014.Maps云端/tech-首页预览与任务创建.md)。不增加删除、取消或重跑操作。
...
- 路由：`/dashboard/` 为 Online 创建与历史，`/dashboard/api/` 为 API，`/dashboard/subscriptions/` 为订阅；遵循现有默认语言根路径与非默认语言前缀规则。
...
- 首页预览与正式创建的真实采集、下载及审查证据见 [Online 界面实施清单](../014.Maps云端/plans/004.首页预览与Dashboard创建.md)。工作区认证与支付验证范围仍见下方工作区实施清单。
