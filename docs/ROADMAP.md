...
| A11 | 插件侧账号登录 + 配额展示(对应 007/003;登录 2026-09-01 迁 v3 浏览器身份) | 逆向 06 | 🔍 | ✅ (2026-08-30;2026-09-01 额度基建 MySQL 化——统一 usage 服务三线门面,见 `feat/000.架构/tech-额度基建.md`) |
...
| E3 | 面板 UI(Vue 直插,非 iframe)+ 官网订阅入口 | 同上 §9.1 | ♻️ | ✅ |
...
- 2026-09-06 **网站用户 Dashboard 核心交付(015 plans/003)**:全站导航登录/Dashboard 入口(站级登录弹窗 + 共享会话,`/extension-login/` 桥接 owner 保留)、Vue 工作区三视图(在线导出历史〔列表/明细/CSV+ZIP 下载〕、API 管理未开放、订阅管理三线 + 渠道管理入口自 Pricing 迁入)、`@astrojs/vue` + vue-tsc 接入构建门禁(build 27 页)。验证:聚焦 e2e(dashboard 6 用例 ×2 project = 12/12 + Pricing 主路径与 landing 6/6)全绿、ui_token_lint 新增文件清零(存量 ClinkReturnPage 4 处裁决不修,全局命令仍失败);module-scripts 3 处断言已适配并定向 3/3 通过;两轮 review 修复(7 项阻塞 + 插件桥 includeSiteAuth 边界、401 重登恢复原视图、Google 回跳/补验证保留目的地可运行验证)后三轮 review 通过(「干的不错」)。本地后端+DB+本地存储替身下载闭环通过(隔离当前源码后端 + seed 身份,auth/me→历史→明细→CSV/ZIP 实际落盘且内容与 DB 一致);外部 R2/AliOSS 端点未验证,用户决定停止验收。细节见 `feat/015.工具与增长/`。
...
- 2026-08-31 **占位入口基线推翻(hydra)**:营销站菜单对标竞品全量补齐——新增 Online Scraper + Scraper/Reviews/Photos API + MCP 落地页 ×5,导航 Online 改真链接 + 新增 API 下拉,首页 Online/API 占位卡改链接;页面全部可打开,页内未落地功能按钮点击无效(无跳转/无请求/不埋点),云端功能本体仍归 014。替代 2026-08-30「Online 入口留空点击无效」裁决。需求落 `015.工具与增长/feat.md` D5 附,计划 `015.工具与增长/plans/002.营销站Online与API落地页.md`(U1 pending)。
