#!/usr/bin/env python3
# E12c: ll 受控实验(711 住宅 sticky) —— 出口固定, 分辨结果地理跟出口还是跟视口
...
def curl_json_rpc(url, sess, cookie):
...
def exit_geo(sess):
    for _ in range(3):
        code, body = curl_raw("http://ip-api.com/json/?fields=country,city,regionName,lat,lon,query,isp", sess)
        try:
            d = json.loads(body)
            if "lat" in d:
                return d
...
def near(lat, lng, ref_lat, ref_lng, deg):
...
def main():
    ...
    out = {}
    for sess in ("e12a", "e12b", "e12c"):
        geo = exit_geo(sess)
        if not geo:
            print(f"[{sess}] exit geo FAIL", flush=True)
            continue
        print(f"[{sess}] exit: {geo.get('city')}, {geo.get('regionName')} @ "
        ...
        for tag, (d, lng, lat) in VIEWPORTS.items():
            ...
            code, body = curl_json_rpc(url, sess, ck)
            ...
            print(f"  [{sess}/{tag}] n={len(recs)} center={center} "
                  f"exit={follows_exit} vp={follows_vp}", flush=True)
            ...
            vp_rows[tag] = {"n": len(recs), "center": center,
                            ...
                            "records": recs}
            time.sleep(1.5)
        out[sess] = {"exit": geo, "viewports": vp_rows}
    verdict = {
        "sessions": len(out),
        "follows_exit_all": all(r["follows_exit"] for v in out.values() for r in v["viewports"].values()),
        "follows_vp_any": any(r["follows_viewport"] for v in out.values() for r in v["viewports"].values()),
    }
    print("VERDICT:", json.dumps(verdict), flush=True)
    json.dump({"sessions": out, "verdict": verdict},
              open("<REDACTED_PATH>/data/e12c_ll_sticky.json", "w"),
              ensure_ascii=False, indent=1)
    print("saved -> e12c_ll_sticky.json", flush=True)
