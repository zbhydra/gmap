...
test.describe('Website Shell', () => {
  ...
  test('footer exposes product, company and legal links', async ({ page }) => {
    ...
    // TG changelog 页已剥离，页脚不再提供入口
    await expect(footer.locator('a[href="/changelog/"]')).toHaveCount(0)
  })
...
test.describe('Secondary Pages', () => {
  ...
  test('retired TG routes no longer exist', async ({ page }) => {
    for (const path of ['/changelog/', '/extension-login/', '/extension-login-v2/', '/extension-login-bing/']) {
      const response = await page.goto(path)
      expect(response?.status(), `expected ${path} to be retired`).toBe(404)
    }
  })
})
